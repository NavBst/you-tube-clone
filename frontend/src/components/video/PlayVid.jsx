...
const PlayVid = ({ vid }) => {
    ...
    const video = vid[0];
    const { channelName, channelAvatar, uploadedAt, title, views } = video;
    // console.log(vids[0])
    const handleLike = () => {
    ...
    return (
        <div className="w-full">
            ...
            <div className="w-full aspect-video bg-black rounded-xl overflow-hidden">
                <video
                    src={'https://videos.pexels.com/video-files/32730662/13953891_2560_1440_24fps.mp4'}
                    controls
                    autoPlay
            ...
            <div className='flex justify-between'>
                ...
                <div className='flex gap-2'>
                    <div className="flex h-10  bg-slate-200 rounded-full">
                        <button className="px-4 py-1.5 text-sm rounded-l-full hover:bg-slate-300 flex items-center after:content-none after:h-8 after:w-1 after:bg-amber-300 cursor-pointer" onClick={() => handleLike()}>
                            {isLiked ? <AiFillLike /> : <AiOutlineLike />}  4.5k
                        </button>
                        <div className="w-[1px] h-6  bg-gray-400 self-center"></div>
                        <button className="px-4 py-1.5 bg-slate-200 text-sm rounded-r-full hover:bg-slate-300 cursor-pointer" onClick={() => { handleDislike() }}>
